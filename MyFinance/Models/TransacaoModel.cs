...
namespace MyFinance.Models
{
    public class TransacaoModel
    {
        ...
        public void Insert()
        {
            ...
            if (Id == 0)
            {
                sql = $"INSERT INTO TRANSACAO (DATA, TIPO, VALOR, DESCRICAO,  CONTA_ID, PLANO_CONTAS_ID, USUARIO_ID) VALUES ('{DateTime.Parse(Data).ToString("yyyy/MM/dd")}','{Tipo}','{Valor}','{Descricao}','{Conta_Id}','{Plano_Contas_Id}','{id_usuario_logado}')";
            }
            else
            {
                sql = $"UPDATE TRANSACAO SET DATA ='{DateTime.Parse(Data).ToString("yyyy/MM/dd")}', DESCRICAO='{Descricao}', TIPO='{Tipo}', VALOR='{Valor}', CONTA_ID='{Conta_Id}', PLANO_CONTAS_ID='{Plano_Contas_Id}' WHERE USUARIO_ID = '{id_usuario_logado}' AND ID='{Id}'";
            }
    ...
    public class Dashboard
    {
        ...
        public List<Dashboard> RetornarDadosGraficoPie()
        {
            ...
            string sql = "select sum(t.valor) as total, p.Descricao from transacao as t inner join plano_contas as p " +
                "on t.Plano_Contas_Id = p.Id where t.Tipo = 'Despesa' group by p.Descricao;";
